...
void H264FileSink::sendFrame(MediaFrame* frame)
{
    ...
    RtpHeader* rtpHeader = mRtpPacket.mRtpHeader; // 获取RTP数据包的头部指针
    uint8_t naluType = frame->mBuf[0];

    if (frame->mSize <= RTP_MAX_PKT_SIZE)
    {
        memcpy(rtpHeader->payload, frame->mBuf, frame->mSize);// 将帧数据拷贝到RTP数据包的负载中
        mRtpPacket.mSize = RTP_HEADER_SIZE + frame->mSize; //设置RTP数据包的大小
    ...
    else // 如果帧大小大于最大包大小
    {
        ...
        for (i = 0; i < pktNum; i++)
        {
            ...
            * */
            ...
            if (i == 0) //第一包数据
                rtpHeader->payload[1] |= 0x80; // 设置start标志位
            else if (remainPktSize == 0 && i == pktNum - 1) //最后一包数据
                rtpHeader->payload[1] |= 0x40; // 设置end标志位
